...
description: "Set custom call outcome fields for a call using the public frontend API."
...
1. **Phone Number Format**: Phone numbers can be provided in E.164 format (e.g., `+14155551234`) or with formatting characters (e.g., `+1 (555) 123-4567`). The API automatically strips spaces, hyphens, parentheses, and dots before validation, so both formats are treated equivalently. The `+` sign must be included in the URL path.
