...
The **Test from here** feature lets you run and validate a call flow starting from any block, without triggering the entire workflow from the beginning. This makes it much faster to debug logic, verify variable handling, and confirm integrations while you’re actively building or refining a flow.

### What “Test from here” does

Normally, testing a flow means starting from the trigger and walking through every step before reaching the area you want to verify. **Test from here** removes that friction. It allows you to jump directly into a specific node, such as an intake block, lookup step, or escalation path, and simulate how the agent behaves from that exact point onward.

This is especially useful for long or complex flows where re-testing from the start would slow you down.       

### How to start a test from a specific block
