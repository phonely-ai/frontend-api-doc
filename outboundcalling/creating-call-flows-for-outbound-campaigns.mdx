...
<Steps>
  ...
  <Step title="Add Flow Components">
    ...
    Outbound workflows also support pre-call actions above the **Make Call** block. Use them to prepare data before Phonely places the outbound call, such as calling an API, running code, applying a filter, or sending a setup SMS when your workflow requires it.

    Only one pre-call chain can connect into the **Make Call** block. Connect the final pre-call action to the top handle of **Make Call**, then continue building the answered-call path from the bottom handle.
  </Step>
