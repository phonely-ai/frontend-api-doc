...
For example, inside an API Request block, set a parameter's source to "Auto-gather," choose a type (Text, Email, Phone, Date, Time), and mark it required. The agent will ask at runtime and then proceed. [Learn more about auto-gather variables here](/key-concepts/auto-gather-variables).
