...
<Steps>
  ...
  <Step titleSize="h3" title="Routing">
    ...
    This section controls what Phonely should do when the outbound number does not connect cleanly to a human, especially when the destination is an IVR menu (e.g., “Press 1 for Sales”) or voicemail.
    ...
    **Example:**\
    You are routing VIP calls to an account manager’s direct line.\
    If that line forwards to a company IVR after hours, you may not want the caller trapped in a phone tree.\
    Set **Exit on IVR Detection** so the flow can return and handle the caller with a fallback message.

    ### 2) IVR Detection Prompt
