...
**Meets condition (True path)** → Executes the next connected block when the configured condition logic evaluates to true, meaning the required condition outcome is achieved according to the filter configuration.

**Does not meet condition (False path)** → Executes the next connected block when the configured condition logic evaluates to false, meaning the required condition outcome is not achieved according to the filter configuration.

<Frame>
  <img
    src="/assets/list-of-filter-options.png"
    alt="List Of Filter Options"
    title="List Of Filter Options"
    style={{ width:"93%" }}
  />
</Frame>
...
Under **Flow Actions**, choose **Filter**. The Filter block will appear with two output paths: **True** and **False**.

### Define Your Conditions

Each condition is a rule that tells Phonely what to check.

Click the filter block to open its configuration panel. Under **Conditions**, click into **Condition 1**.
...
<Steps>
  ...
  <Step title="Doesn’t exist:">
    True when the variable is empty, null, or was never set.
  </Step>
  <Step title="is equal to">
    Compares the field to a specific value.
  </Step>
  <Step title="is not equal to">
    True when the field does not match the provided value
  </Step>
