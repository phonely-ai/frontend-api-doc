import { inspectImpact } from './docs-impact.mjs';
...
export function validateReviewDate(before, after, reviewDay, affected = false) {
  if (!affected && body(before) === body(after)) return;
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  ...
  const impact = inspectImpact(process.cwd(), base);
  const files = impact.pages.map(page => `${page}.mdx`);
  ...
  for (const file of files) {
    ...
    try { validateReviewDate(before, readFileSync(file, 'utf8'), reviewDay, (impact.reasons[file] ?? []).some(reason => reason !== 'direct change')); }
